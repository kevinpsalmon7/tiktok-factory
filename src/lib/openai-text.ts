...
export async function extractIntent(prompt: string, apiKey: string, log?: Logger): Promise<CarouselIntent> {
  ...
  await log?.({ step: 'chatgpt.intent.response', message: 'extractIntent: raw ChatGPT response', payload: { raw: rawText } })
  ...
  try {
    ...
    await log?.({ step: 'chatgpt.intent.parsed', message: `intent: ${count} carousel(s)`, payload: { count, perCarousel: carousels } })
    return { count, perCarousel: carousels }
  } catch (err) {
    await log?.({ step: 'chatgpt.intent.parsed', message: 'intent parse failed, falling back to count=1', level: 'warn', payload: { error: err instanceof Error ? err.message : String(err) } })
...
export async function generateCarousels({
  ...
  const systemPrompt = `You generate content for TikTok/Instagram carousels.
...
- All text in "text_fields" is what will be rendered on the slide.`
  ...
  const imageContents: ImageUrlContent[] = images.map(img => ({
  ...
  const messageContents: MessageContent[] = [
    ...imageContents,
    { type: 'text' as const, text: userMessage + imagesNote },
  ]

  await log?.({
    step: `chatgpt.carousel.request${carouselTag ? '.' + carouselTag : ''}`,
    message: `generateCarousels: prompt sent to ChatGPT (${count} carousel(s), ${images.length} image(s))`,
    payload: { model, count, tag: carouselTag, imageCount: images.length, systemPrompt, userMessage },
  })
  ...
  await log?.({
    step: `chatgpt.carousel.response${carouselTag ? '.' + carouselTag : ''}`,
    message: 'generateCarousels: raw ChatGPT response',
    payload: { raw, usage: completion.usage },
  })
  ...
  // Wrap in array if model returned a single object
  const result = Array.isArray(parsed) ? parsed : (parsed.carousels ?? [parsed])

  await log?.({
    step: `chatgpt.carousel.parsed${carouselTag ? '.' + carouselTag : ''}`,
